...
public class ArchiveHandlerTest {
    ...
    public void fileRecognitionTest() throws IOException, ArchiveHandlerException {
        ...
        Assertions.assertThrows(ArchiveHandlerException.class,
                () -> archiveHandler.probeContentAndUnarchive(testPath, openArchive(TEST_DIR + "CORRUPTED_ZIP.zip")));

        Assertions.assertEquals(expected,
                archiveHandler.probeContentAndUnarchive(testPath, openArchive(TEST_DIR + "ZIP.zip")));

        Assertions.assertEquals(expected,
                archiveHandler.probeContentAndUnarchive(testPath, openArchive(TEST_DIR + "TAR.tar")));
        Assertions.assertEquals(expected,

                archiveHandler.probeContentAndUnarchive(testPath, openArchive(TEST_DIR + "TAR_BZIP2.tar.bz2")));

        Assertions.assertEquals(expected,
                archiveHandler.probeContentAndUnarchive(testPath, openArchive(TEST_DIR + "TAR_XZ.tar.xz")));

        Assertions.assertEquals(expected,
                archiveHandler.probeContentAndUnarchive(testPath, openArchive(TEST_DIR + "TAR_GZIP.zip")));

        Assertions.assertEquals(expected,
                archiveHandler.probeContentAndUnarchive(testPath, openArchive(TEST_DIR + "TAR_BZIP2.zip")));

        Assertions.assertEquals(expected,
                archiveHandler.probeContentAndUnarchive(testPath, openArchive(TEST_DIR + "ZIP.tar.gz")));

        Assertions.assertEquals(expected,
                archiveHandler.probeContentAndUnarchive(testPath, openArchive(TEST_DIR + "TAR_GZIP.tar.gz")));
    }
    ...
    public void multipleFilesExtraction() throws IOException, ArchiveHandlerException {
        ...
        Assertions.assertEquals(expected,
                archiveHandler.probeContentAndUnarchive(testPath, openArchive(TEST_DIR + "MTAR.tar.bz2")));
    }
}
